    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            for ((index, entry) in sortedHooks.withIndex()) {
                ...
                val contentText =
                    if (hookName == "card_injection" && cardNames.isNotEmpty()) {
...
private fun hookTargetName(hookName: String): String =
    when (hookName) {
...
@Composable
private fun hookDisplayName(hookName: String): String =
    when (hookName) {
...
@Composable
private fun processDisplayName(process: String): String =
    when (process) {

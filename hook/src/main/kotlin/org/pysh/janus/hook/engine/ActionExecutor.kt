...
import org.pysh.janus.core.model.HookAction
import org.pysh.janus.core.model.HookRule
import org.pysh.janus.core.model.HookTarget
            ...
            if (target.methodName == null) {
                ...
                val method = if (paramClasses != null) {
                    ...
                    if (candidates.isEmpty()) {
                        throw NoSuchMethodException("${target.className}.${target.methodName}")
                    }
                    if (candidates.size > 1) {
                        Log.w(TAG, "Multiple overloads for ${target.className}.${target.methodName} " +
                            "(${candidates.size} found), using first. Consider specifying paramTypes.")
                    }
                    candidates.first()
                }
                val hooker = createHooker(target.id, target.action, rule, config)

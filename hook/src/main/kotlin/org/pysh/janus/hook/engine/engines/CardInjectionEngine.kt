...
class CardInjectionEngine : HookEnginePlugin {
    ...
    companion object {
        ...
        private const val JANUS_BASE = "/data/system/theme_magic/users/0/subscreencenter/janus"
    ...
    private fun patchConstants(classLoader: ClassLoader, activeBusinesses: Set<String>) {
        try {
            ...
            ReflectUtils.setStaticField(cls, fieldA, HashMap(origA).apply { put(JANUS_PKG, "weather") })
            @Suppress("UNCHECKED_CAST")
            val origC = ReflectUtils.getStaticField(cls, fieldC) as Map<String, Set<String>>
            ReflectUtils.setStaticField(cls, fieldC, HashMap(origC).apply { put(JANUS_PKG, activeBusinesses) })
            @Suppress("UNCHECKED_CAST")
            val origD = ReflectUtils.getStaticField(cls, fieldD) as Map<String, String>
            ReflectUtils.setStaticField(cls, fieldD, HashMap(origD).apply {
                for (biz in activeBusinesses) { put(biz, "$TEMPLATE_BASE/$biz") }
            })
            Log.d(TAG, "Constants patched: $activeBusinesses")
        } catch (e: Throwable) { Log.e(TAG, "patchConstants: ${e.message}") }
    }
    ...
    private fun patchProtectedMap(classLoader: ClassLoader) {
        ...
        } catch (e: Throwable) { Log.e(TAG, "patchProtectedMap: ${e.message}") }
    }

    private fun hookFilter(module: XposedInterface, classLoader: ClassLoader) {
        try {
            val cls = classLoader.loadClass(targets["filter_class"]!!)
            val method = cls.getDeclaredMethod(targets["filter_method"]!!, String::class.java, java.util.Set::class.java, java.util.Map::class.java)
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                val pkg = chain.args[0] as? String
                if (pkg == JANUS_PKG) {
                    HookStatusReporter.reportBehavior("card_filter", JSONObject().apply { put("action", "filter_bypass"); put("package", pkg) })
                    return@Hooker true
                }
                chain.proceed()
            })
        } catch (e: Throwable) { Log.e(TAG, "hookFilter: ${e.message}") }
    }

    private fun hookAppList(module: XposedInterface, classLoader: ClassLoader, activeBusinesses: Set<String>) {
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                ...
                @Suppress("UNCHECKED_CAST")
                (ReflectUtils.getField(chain.thisObject, fieldR) as ConcurrentHashMap.KeySetView<String, *>).add(JANUS_PKG)
                @Suppress("UNCHECKED_CAST")
                val q = ReflectUtils.getField(chain.thisObject, fieldQ) as ConcurrentHashMap<String, Boolean>
                q[JANUS_PKG] = true
                for (biz in activeBusinesses) { q["${JANUS_PKG}_$biz"] = true }
                HookStatusReporter.reportBehavior("card_app_list", JSONObject().apply { put("action", "inject_app_list"); put("businesses", activeBusinesses.size) })
                result
            })
        } catch (e: Throwable) { Log.e(TAG, "hookAppList: ${e.message}") }
    }
    ...
    private fun hookCardSorting(module: XposedInterface, classLoader: ClassLoader) {
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain -> val result = chain.proceed(); sortJanusCards(); result })
            Log.d(TAG, "Card sorting hook installed")
        } catch (e: Throwable) { Log.e(TAG, "hookCardSorting: ${e.message}") }
    }
        ...
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                ...
                for (i in 0 until cards.length()) {
                    ...
                    uiHandler.postDelayed({ injectCard(slot, business, priority, classLoader) }, 3_000L + i * 500L)
                    val periodic = object : Runnable {
                        override fun run() { injectCard(slot, business, priority, classLoader); uiHandler.postDelayed(this, refreshMs) }
                    }
    ...
    private fun sortJanusCards() {
        ...
        val list = try { ReflectUtils.getField(mgr, listFieldName) as? ArrayList<Any> } catch (_: Throwable) { null } ?: return
        val janusIndices = mutableListOf<Int>()
        val janusEntries = mutableListOf<Any>()
        for (i in list.indices) {
            val entry = list[i]
            val key = try { ReflectUtils.getField(entry, keyFieldName) as? String } catch (_: Throwable) { null } ?: continue
            if (JANUS_PKG in key) { janusIndices.add(i); janusEntries.add(entry) }
        }
        if (janusEntries.size < 2) return
        janusEntries.sortBy { ReflectUtils.getIntField(it, priorityFieldName) }
        for (i in janusIndices.indices) { list[janusIndices[i]] = janusEntries[i] }
    ...
    private fun injectCard(slot: Int, business: String, priority: Int, classLoader: ClassLoader) {
        ...
        try {
            val rearParam = JSONObject().apply { put("business", business); put("index", 0); put("priority", priority) }
            val bundle = Bundle().apply { putString("miui.rear.param", rearParam.toString()) }
            ...
            val mgrCls = classLoader.loadClass(targets["manager_class"]!!)
            val ctor = runnableCls.getDeclaredConstructor(mgrCls, Int::class.javaPrimitiveType, String::class.java, String::class.java, Bundle::class.java)
        ...
        } catch (e: Throwable) { Log.e(TAG, "injectCard slot=$slot: ${e.message}") }
    }
    ...
    private fun deployTemplate(slot: Int, business: String) {
        try {
            ...
            if (!srcFile.exists()) { Log.d(TAG, "deployTemplate: source not found: ${srcFile.absolutePath}"); return }
            if (destFile.exists()) { if (destFile.isDirectory) destFile.deleteRecursively() else destFile.delete() }
        ...
        } catch (e: Throwable) { Log.e(TAG, "deployTemplate slot=$slot: ${e.message}") }
    }

    private fun readConfig(): JSONObject? {
        return try { val file = File(CONFIG_PATH); if (!file.exists()) return null; JSONObject(file.readText()) }
        catch (e: Throwable) { Log.e(TAG, "readConfig: ${e.message}"); null }
    }
}

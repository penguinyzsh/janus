...
class WhitelistEngine : HookEnginePlugin {
    companion object { private const val TAG = "Janus-Whitelist" }
    ...
    override fun install(module: XposedInterface, rule: HookRule, classLoader: ClassLoader, config: SharedPreferences) {
        ...
        } catch (e: Throwable) { Log.e(TAG, "install failed: ${e.message}"); HookStatusReporter.report("whitelist", false, e.message) }
    }

    private fun hookCheckMethod(module: XposedInterface, targetClass: Class<*>, methodName: String, config: SharedPreferences) {
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                ...
                HookStatusReporter.reportBehavior("whitelist_check", JSONObject().apply { put("action", "check_in_set"); put("checked", packageName); put("in_set", inSet) })
                if (inSet) { Log.d(TAG, "Allowed: $packageName"); true } else { result }
            })
        } catch (e: Throwable) { Log.e(TAG, "hookCheckMethod: ${e.message}") }
    }

    private fun hookGetMethod(module: XposedInterface, targetClass: Class<*>, methodName: String, config: SharedPreferences) {
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                ...
                HookStatusReporter.reportBehavior("whitelist_merge_set", JSONObject().apply { put("action", "merge_set"); put("merged_count", customWhitelist.size) })
                @Suppress("UNCHECKED_CAST")
                try { (original as MutableCollection<String>).addAll(customWhitelist); original }
                catch (_: Throwable) {
                    val merged = HashSet<String>()
                    if (original is Collection<*>) { @Suppress("UNCHECKED_CAST") merged.addAll(original as Collection<String>) }
                    merged.addAll(customWhitelist); merged
                }
            })
        } catch (e: Throwable) { Log.e(TAG, "hookGetMethod: ${e.message}") }
    }

    private fun hookMusicControllerPackageList(module: XposedInterface, classLoader: ClassLoader, className: String, methodName: String, config: SharedPreferences) {
        try {
            ...
            module.hook(method).intercept(XposedInterface.Hooker { chain ->
                ...
                if (list != null) {
                    ...
                    for (pkg in whitelist) { if (pkg !in list) list.add(pkg) }
                    if (whitelist.isNotEmpty()) {
                        HookStatusReporter.reportBehavior("whitelist_merge_list", JSONObject().apply { put("action", "merge_list"); put("merged_count", whitelist.size) })
                        Log.d(TAG, "Injected ${whitelist.size} packages into MusicController")
                    }
                }
                chain.proceed()
            })
        } catch (e: Throwable) { Log.e(TAG, "hookMusicControllerPackageList: ${e.message}") }
    }
    ...
    private fun getCustomWhitelist(config: SharedPreferences): Set<String> {
        ...
        } catch (e: Throwable) { Log.e(TAG, "Failed to read whitelist: ${e.message}"); emptySet() }
    }
}
